...
int main(int argc, char* argv[], char* envp[]) {
  ...
  int long_index = 0;
  while ((options = getopt_long(argc, argv, "e:f:hv\0", long_options, &long_index)) != -1) {
  ...
  case 'f' : strcpy(txtfile, optarg);                       // display the first line of a *.txt file
      break;
  case 'h' : helpPrinted = true;                            // display help:
      break;
  case 'v' : print_env(envp);                               // enviroment variables
      break;
      //case '\0': printf("No valid program option chosen\n\n");  // No arguments given
      //  break;
  default  : printf("No valid program option chosen\n\n");  // If no arguments are given, print help
	print_help();
	exit(EXIT_FAILURE);                              
    };
  };
