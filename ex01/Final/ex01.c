...
int main(int argc, char* argv[], char* envp[]) {
  ...
  case 'e' : 
      lastLine = true; 
      strcpy(txtfile, optarg);         // display the last line of a *.txt file
      break;
  case 'f' : 
      strcpy(txtfile, optarg);         // display the first line of a *.txt file
      break;
  case 'h' : 
      print_help(); 
      exit(EXIT_SUCCESS);              // display help:
      break;
  case 'v' : 
      print_env(envp); 
      exit(EXIT_SUCCESS);              // enviroment variables
      break;
  default  : 
      print_help(); 
      exit(EXIT_SUCCESS);              // If no arguments are given, print help
      break;                                     
    };
  };
...
/* exit(EXIT_SUCCESS) in de switch-case functie is "niet sjiek" */